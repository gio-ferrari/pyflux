...
class PicoHarp300(LibraryDriver):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mode = (
            MODE_T3  # set T2 or T3 here, observe suitable Syncdivider and Range!
        )
    ...
    def getHardwareInfo(self):
        self.lib.PH_GetHardwareInfo(
            DEV_NUM, self.hwModel, self.hwPartno, self.hwVersion
        )
    ...
    def setup_ph300(self):
        ...
        self.lib.PH_SetSyncDiv(ctypes.c_int(DEV_NUM), ctypes.c_int(self.syncDivider))

        self.lib.PH_SetInputCFD(
    ...
    def binning(self, value):
        self.lib.PH_SetBinning(ctypes.c_int(DEV_NUM), ctypes.c_int(value))
    ...
    @Feat
    def resolution(self):
        self.lib.PH_GetResolution(ctypes.c_int(DEV_NUM), byref(self.res))
    ...
    def countrate(self, channel):
        if channel == 0:
            self.lib.PH_GetCountRate(
                ctypes.c_int(DEV_NUM), ctypes.c_int(0), byref(self.countRate0)
            )
            value = self.countRate0.value
        elif channel == 1:
            self.lib.PH_GetCountRate(
                ctypes.c_int(DEV_NUM), ctypes.c_int(1), byref(self.countRate1)
            )
    ...
    def startTTTR(self, outputfilename):
        ...
        while meas is True:
            ...
            if self.nactual.value > 0:
                ...
                outputfile.write(
                    (ctypes.c_uint * self.nactual.value)(
                        *self.buffer[0 : self.nactual.value]
                    )
                )
